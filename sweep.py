...
class Utilization:
    def __init__(self, err: str = '', bram: float = -1, dsp: float = -1, ff: float = -1, lut: float = -1, latency: float = -1, latency_unit: str = '?'):
        self.bram = bram
        self.dsp = dsp
        self.ff = ff
        self.lut = lut
...
def work(config: Config, con: Connection, logs_dir: str, log_file: IO) -> Utilization:
    ...
    try:
        ...
        return Utilization(err='error: failed to create remote folder')
    ...
    if error is None:
        ...
        for source in SOURCE_FILES:
            try:
                ...
                error = f'error: cannot copy to remote: {e}'
    ...
    if error is None:
        ...
        try:
            ...
            # download the compressed reports file
            con.get(f'{r_tmp_dir}/reports.tar.gz', f'{work_dir}/')

            # copy synth file to the logs
            con.get(f'{r_tmp_dir}/blis_hls/hls/syn/report/blis_csynth.xml', f'{work_dir}/')
        except Exception as e:
            end = time.time()

            print(f'config: {config_desc}: failed, elapsed: {end - start}s, generated by: {con}, exception: {e}')
            error = f'error: {e}'

    # delete the temp directory
    try:
        res = con.run(f'docker run --rm -v {r_tmp_dir}:/opt/data xilinx /bin/bash -c "rm -rf /opt/data/.Xil && rm -rf /opt/data/*" && rm -rf {r_tmp_dir}')
        ...
        error = f'error: cannot delete remote files {e}'

    # Open the synthesis reports
    if Path(f'{work_dir}/blis_csynth.xml').is_file():
        ...
        return Utilization('Success', bram, dsp, ff, lut, latency, latency_unit)
    else:
        return Utilization(err=base64.b64encode(error.encode()))
...
def run(key_file_path:str, logs_path: str, results_path: str):
    ...
    with open(results_path, 'w') as results_file:
        ...
        results_file.write('kc,mc_f,nc_f,ms_f,ns_f,mr,nr,bram,dsp,ff,lut,latency,latency_units,err\n')

        for config, utilization in results:
            results_file.write(f'{config.kc},{config.mc_f},{config.nc_f},{config.ms_f},{config.ns_f},{config.mr},{config.nr},{utilization.bram},{utilization.dsp},{utilization.ff},{utilization.lut},{utilization.latency},{utilization.latency_unit},{utilization.err}\n')
